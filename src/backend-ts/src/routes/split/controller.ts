...
router.get("/getSplits", async (req, res) => {
  try {
  ...
  const rows = (await mysql).query(q, chatroomID);
  } catch (error) {
...
router.post("/addSplits", async (req, res) => {
  ...
  const q = `insert into (\`title\`, \`super_cid\`, \`payer\`, \`attendees_ids\`, \`price\`, \`issplited\`) value
  (?, ?, ?, ?, ?, FALSE)`
...
router.post("/startSplits", async (req, res) => {
  ...
  const rows = (await mysql).query(q, chatroomID); 
  const debts= [];
  (await rows).forEach(({acid, title, payer, attendees, price, issplited}) => {
